...
class ModernAutoPartsERP(ctk.CTk):
    ...
    def show_login_page(self):
        ...
        ctk.CTkLabel(cashier_card, text="POS & View-Only Inventory", font=ctk.CTkFont(size=11), text_color="gray").pack(
            pady=(0, 10))
        ...
        ctk.CTkLabel(owner_card, text="Full Administrative Privileges", font=ctk.CTkFont(size=11),
    ...
    # --- OWNER ACCESS HANDLER & VIEW ---
    def handle_owner_access_click(self):
        if self.current_role == "owner":
            self.show_owner_settings_view()
        else:
            self.show_owner_login_view()

    def show_owner_login_view(self, pending_view_callback=None):
        ...
        ctk.CTkLabel(dialog, text="Enter owner password to unlock admin privileges.", font=ctk.CTkFont(size=12),
                     text_color="gray").pack(pady=(0, 15))

        pwd_frame, pwd_entry = self.create_password_field(dialog,
                                                          "e.g. owner_1234 (min 8 chars, 1 num, 1 spec @,*,_,-)",
                                                          width=350)
        ...
        ctk.CTkButton(dialog, text="Authenticate 🔓", fg_color="#1f6aa5", height=36, width=340,
                      font=ctk.CTkFont(weight="bold"), command=attempt_login).pack(pady=15)

    def show_owner_settings_view(self):
        """View with tabs allowing the owner to update owner & cashier passwords after verifying the current password."""
        dialog = ctk.CTkToplevel(self)
        ...
        ctk.CTkLabel(tab_owner, text="Update Owner Access Password", font=ctk.CTkFont(size=14, weight="bold")).pack(
            pady=(10, 5))

        ctk.CTkLabel(tab_owner, text="Current Owner Password:", font=ctk.CTkFont(size=11), text_color="gray").pack(
            anchor="w", padx=30, pady=(5, 2))
        ...
        ctk.CTkLabel(tab_owner, text="New Owner Password:", font=ctk.CTkFont(size=11), text_color="gray").pack(
            anchor="w", padx=30, pady=(5, 2))
        new_o_frame, new_o_pwd = self.create_password_field(tab_owner,
                                                            "e.g. owner_1234 (min 8 chars, 1 num, 1 spec @,*,_,-)",
                                                            width=340)
        ...
        ctk.CTkButton(tab_owner, text="Save New Owner Password", fg_color="#1f6aa5", command=update_owner_pwd).pack(
            pady=12)

        # --- Tab 2: Cashier Password Change ---
        ctk.CTkLabel(tab_cashier, text="Update Cashier Login Password", font=ctk.CTkFont(size=14, weight="bold")).pack(
            pady=(10, 5))

        ctk.CTkLabel(tab_cashier, text="Current Cashier Password:", font=ctk.CTkFont(size=11), text_color="gray").pack(
            anchor="w", padx=30, pady=(5, 2))
        ...
        ctk.CTkLabel(tab_cashier, text="New Cashier Password:", font=ctk.CTkFont(size=11), text_color="gray").pack(
            anchor="w", padx=30, pady=(5, 2))
        ...
        ctk.CTkButton(tab_cashier, text="Save New Cashier Password", fg_color="#2ba84a",
                      command=update_cashier_pwd).pack(pady=12)
    ...
    def show_presets_view(self):
        if self.current_role != "owner":
            self.show_owner_login_view(self.show_presets_view)
            return
        self.clear_main_frame()
    ...
    def show_sales_view(self):
        if self.current_role != "owner":
            self.show_owner_login_view(self.show_sales_view)
            return
        self.clear_main_frame()
